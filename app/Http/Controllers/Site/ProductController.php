...
class ProductController extends Controller
{
    ...
    public function get_view(Request $request, $id)
    {
        ...
        return view('site.product.view', compact(
            'product',
            'farm',
            'trending',
            'related',
    ...
    public function get_filter(Request $request)
    {
        ...
        $products = Product::where('is_status', 1)
            ...
            ->when(!empty($selectedOffers), function ($query) use ($selectedOffers) {
                // Adjust offer filter to be an OR condition
                $query->where(function ($q) use ($selectedOffers) {
                    foreach ($selectedOffers as $offer) {
                        if ($offer == 5) {
                            $q->orWhere('offer', '<=', 5);
                        } elseif ($offer == 10) {
                            $q->orWhereBetween('offer', [5, 10]);
                        } elseif ($offer == 15) {
                            $q->orWhereBetween('offer', [10, 15]);
                        } elseif ($offer == 25) {
                            $q->orWhereBetween('offer', [15, 25]);
                        } elseif ($offer == 100) {
                            $q->orWhere('offer', '>', 25);
                        }
                    }
